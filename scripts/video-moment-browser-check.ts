...
export async function checkVideoMomentBrowserJourney(): Promise<BrowserJourneyFacts> {
  ...
  try {
    ...
    const searchOutput = await cli(
      ...
      'run-code',
      playwrightPageFunction(
        `const input = page.getByLabel('What explanation do you remember?'); await input.fill('${VIDEO_MOMENT_QUERY}'); await input.press('Enter'); const cards = page.locator('[data-client-results] article'); await cards.first().waitFor({ state: 'visible' }); const first = cards.first(); const anchor = first.locator('h3').getByRole('link'); return 'AI_MOMENT_SEARCH:' + JSON.stringify({ query: await input.inputValue(), visibleResultCount: await cards.count(), firstMomentId: await first.getAttribute('data-moment-id'), anchorHref: await anchor.getAttribute('href') });`,
      ),
    );
