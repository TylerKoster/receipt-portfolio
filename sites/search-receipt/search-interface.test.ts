...
describe('Search Receipt query and offer adapter', () => {
  ...
  it('creates one accessible fragment-only link that follows the applied filters', () => {
    type Listener = (event: { preventDefault(): void }) => void;
    interface FakeElement {
      addEventListener(type: string, listener: Listener): void;
      append(child: FakeElement): void;
    ...
    expect(initializeSearchReceipt(root)).toBe(true);
    const link = elements.get('[data-search-share-link]');
    expect(link?.textContent).toBe('Link to this filtered view');
    const expectOnlyShareLink = (fragment: string) => {
      const links = [...elements.entries()].filter(
        ([selector]) => selector === '[data-search-share-link]',
      );

      expect(links).toHaveLength(1);
      expect(link?.attributes.get('href')).toBe(fragment);
      expect(link?.attributes.get('href')).toMatch(
        /^#search\?query=[^#]*&topic=[^#]*$/u,
      );
    };
    expectOnlyShareLink('#search?query=&topic=');

    query.value = 'Crawling status';
    topic.value = 'search-status';
    form.listeners
      .get('submit')
      ?.forEach((listener) => listener({ preventDefault() {} }));
    expectOnlyShareLink('#search?query=Crawling+status&topic=search-status');
    query.value = 'service & status';
    query.listeners
      .get('input')
      ?.forEach((listener) => listener({ preventDefault() {} }));
    expectOnlyShareLink('#search?query=service+%26+status&topic=search-status');
    topic.value = 'guidance';
    topic.listeners
      .get('change')
      ?.forEach((listener) => listener({ preventDefault() {} }));
    expectOnlyShareLink('#search?query=service+%26+status&topic=guidance');
    elements
    ...
    expectOnlyShareLink('#search?query=&topic=');
    expect(initializeSearchReceipt(root)).toBe(true);
    expect(elements.get('[data-search-share-link]')).toBe(link);
  });

  it('restores a namespaced fragment filter and ignores unrelated or malformed fragments', () => {
    type Listener = (event: { preventDefault(): void }) => void;
    interface FakeElement {
      addEventListener(type: string, listener: Listener): void;
      append(child: FakeElement): void;
    ...
    const restored = createRoot('#search?query=crawling&topic=search-status');
    expect(initializeSearchReceipt(restored.root)).toBe(true);
    expect(restored.query.value).toBe('crawling');
    expect(restored.topic.value).toBe('search-status');
    expect(restored.cards.map((card) => card.hidden)).toEqual([false, true]);
    expect(
      restored.elements.get('[data-search-share-link]')?.attributes.get('href'),
    ).toBe('#search?query=crawling&topic=search-status');

    const encoded = createRoot(
      '#search?query=service%20%26%20status&topic=search-status',
    );
    expect(initializeSearchReceipt(encoded.root)).toBe(true);
    expect(encoded.query.value).toBe('service & status');
    expect(encoded.topic.value).toBe('search-status');
    expect(encoded.cards.map((card) => card.hidden)).toEqual([true, true]);

    for (const hash of [
      '#search',
      '#other?query=crawling',
      ...
      const defaultState = createRoot(hash);
      expect(initializeSearchReceipt(defaultState.root)).toBe(true);
      expect(defaultState.query.value).toBe('');
      expect(defaultState.topic.value).toBe('');
      expect(defaultState.cards.map((card) => card.hidden)).toEqual([
        false,
        false,
      ]);
    }
