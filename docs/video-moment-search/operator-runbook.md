...
The current implementation queue is the coordinator-supplied September 2 [Owned Video Library demonstration PRD](C:/Users/Tkost/Documents/ChatGPT/SEO/docs/product/2026-09-02-owned-video-library-demo-prd.md) §14 and its [timeboxed task plan](C:/Users/Tkost/Documents/ChatGPT/SEO/docs/superpowers/plans/2026-09-02-owned-library-timeboxed-delivery.md) §§2 and 5. They direct the five usable increments and their timeboxed task ledger for a private, local-only, fictional, transcript-first demonstration using approved openly licensed material. Those documents are draft implementation direction supplied by the coordinator; they have not been integrated into accepted `main`.

The [service experiment ledger](service-experiment-ledger.md) and [pilot-entry review packet](pilot-entry-review.md) are future real-customer pilot-readiness materials only. They are dormant and non-applicable during the current demonstration, grant no current demo authority, and are not prerequisites for it. Their future safety gates remain binding if a separately authorized real-customer pilot is ever considered.

`experiment-ledger.json` and the ranked sequence below are preserved historical public-proof experiments. They supplement, and do not replace, the current coordinator-supplied demonstration direction, the future service ledger, or the released historical public artifact at `sites/video-moment-search/product-experiment-ledger.json`. In particular, the historical 500-moment continuation target does not authorize corpus expansion and cannot outrank the current demonstration, or any future service utility, incumbent-sufficiency, authority, privacy, or paid-pilot gate.
