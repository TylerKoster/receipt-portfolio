...
3. **Researcher relevance (rank 3).** The researcher flow is preview-testable only: enter a concept, inspect a ranked list, and reach the correct exact moment within 60 seconds. Apply the **>=80% top-three relevance** heuristic gate; record timestamp landing error and zero-result review status without calling it usability evidence.
